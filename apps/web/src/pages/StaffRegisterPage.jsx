...
function StaffRegisterPage() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			const response = await authApi.registerHealthOfficer(staffData);
			...
			setError(err.message || "An error occurred during staff registration");
		} finally {
			setLoading(false);
		}
	};
						...
						<div className="min-h-[220px] flex flex-col justify-center">
							{step === 1 && (
								<div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-500">
									<div>
										<label className="text-[11px] font-bold text-slate-700 ml-1 uppercase">I am a:</label>
										<select
											name="role"
											value={formData.role}
							...
							{step === 2 && (
								<div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-500">
									<div className="space-y-1">
										<label htmlFor="email" className="text-[13px] font-bold text-slate-700 ml-1 uppercase">Email address</label>
										<input
											id="email"
											name="email"
											type="email"
											...
											onChange={handleChange}
											className="block w-full rounded-2xl border border-slate-200 bg-white px-5 py-3.5 text-slate-900 placeholder-slate-400 focus:border-teal-500 focus:outline-none focus:ring-4 focus:ring-teal-500/10 transition-all font-medium"
											placeholder="Enter your Email"
										/>
									</div>
									<div className="space-y-1">
										<label htmlFor="phone" className="text-[13px] font-bold text-slate-700 ml-1 uppercase">Phone Number</label>
										<input
											id="phone"
											name="phone"
											type="tel"
							...
							{step === 3 && (
								<div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-500">
									<div className="space-y-1">
										<label htmlFor="password" className="text-[13px] font-bold text-slate-700 ml-1 uppercase">Set Password</label>
										<input
											id="password"
											name="password"
											type="password"
									...
									<div className="p-4 rounded-2xl bg-teal-50 border border-teal-100">
										<p className="text-xs text-teal-600 font-medium leading-relaxed">
											By clicking register, you agree to follow MediLab clinical security protocols and privacy policies.
										</p>
									</div>
								</div>
							)}
						...
						<div className="flex flex-col items-center gap-3">
							...
							<div className="text-center pt-2">
								<p className="text-slate-500 font-medium text-sm">
									...
									<Link to="/staff/login" className="text-teal-600 font-bold hover:text-teal-800 transition-colors">
										Log in
									</Link>
								</p>
							</div>
