...
class HouseholdService {
  ...
  async createHousehold(householdData) {
    ...
    // After saving, household_id is auto-generated by pre-save hook.
    // Update all members belonging to this household with the new household_id.
    if (household.members && household.members.length > 0) {
      await Member.updateMany(
        { member_id: { $in: household.members } },
  ...
  async updateHousehold(id, updateData) {
    ...
    // If members were updated, ensure their records are also synced with the household_id
    if (updateData.members) {
      await Member.updateMany(
        { member_id: { $in: updateData.members } },
        { $set: { household_id: household.household_id } }
      );
    }
